...
class ARViewController: UIViewController, ARSCNViewDelegate, SCNPhysicsContactDelegate {
    ...
    private var wicketNode: SCNNode?
    ...
    func updateScoreNode(score: Int) {
        ...
        scoreNode = textNode
        wicketNode?.addChildNode(textNode)
    ...
    @objc func didReceiveTapGesture(_ sender: UITapGestureRecognizer) {
        ...
        wicketNode?.removeFromParentNode()
        //Grabs scn file and creates a Scene Node object
        guard let wicket = SCNScene(named: "Wicket.scn")?.rootNode else {
            return
        }
        
        //Makes the object bigger/smaller
        wicket.scale = SCNVector3(0.1, 0.1, 0.1)
        
        //Let's you make a new position based on where the user tapped
        //let position2 = SCNVector3((position.x * 2.0), position.y, position.z)
        
        wicket.position = position
        sceneView.scene.rootNode.addChildNode(wicket)
        self.wicketNode = wicket
